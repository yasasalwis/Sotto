...
- **Tools the model can call**: twenty-six built-ins that run entirely on device — dates and time zones, arithmetic and units, percentages and number bases, text rewriting, find-and-replace, extraction, sorting, word counts and diffs, JSON and CSV, descriptive statistics, encoding, hashing, random numbers, a chat search, and delegating a task to a second model session with its own context window — plus your own Google-search and HTTPS-request tools, each with its own approval rule. Four are on out of the box; the rest are one switch away in Tools, because Apple's on-device model has a 4,096-token window and every offered tool spends some of it. A macOS shell-command tool exists for builds distributed outside the App Store.
